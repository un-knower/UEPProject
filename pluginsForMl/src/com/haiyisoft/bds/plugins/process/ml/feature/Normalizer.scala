...
import org.apache.spark.ml.feature.{Normalizer => MLTransformer}
import org.apache.spark.sql.DataFrame

/**
  * Created by XingxueDU on 2018/3/21.
  */
class Normalizer extends DataFeaturesInter{
  /**
  ...
  override def getNecessaryParamList: Array[ParamFromUser] = {
    ...
    val p = ParamFromUser.Double("Normalizer.p","请输入p值#标准化使用的范数位阶").>=(0).fix.addDefaultValue("2")
    Array(input,output,p)
  }

  def getP:Double = getValueParam("Normalizer.p").doubleValue()

  override def getPluginName: String = "正则化处理"
}
